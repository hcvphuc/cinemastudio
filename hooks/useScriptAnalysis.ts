    ...
    const analyzeScript = useCallback(async (
        ...
        try {
            ...
            if (videoZoneConfig?.enabled) {
                ...
                const totalScenes = videoScenes + staticScenes;
                // Video zone duration = videoScenes × 8 seconds per clip
                const videoDurationSec = videoScenes * 8;
                const videoDurationMin = videoDurationSec / 60;
                // Word budget based on actual duration, not scene count ratio
                const videoWordBudget = Math.round(videoDurationMin * wpm);
                const staticWordBudget = Math.max(0, wordCount - videoWordBudget);
                const wordsPerVideoScene = Math.ceil(videoWordBudget / videoScenes);
                const wordsPerStaticScene = staticScenes > 0 ? Math.ceil(staticWordBudget / staticScenes) : 0;
                videoZoneInstructions = `
*** VIDEO ZONE SPLIT — CRITICAL REQUIREMENT ***

🚨 RULE #0 — COMPLETE SCRIPT COVERAGE (HIGHEST PRIORITY):
- The ENTIRE script text must be distributed across ALL ${totalScenes} scenes.
- Every word of the script must appear in exactly one scene's voiceOverText.
- NO text may be skipped or omitted. If you run out of scenes, MERGE remaining text into the last scenes.
- After generating, verify: concatenating all voiceOverText fields reproduces the full original script.

🚨 RULE #1 — WORD BOUNDARY (HOW TO SPLIT THE SCRIPT):
- The script has ~${wordCount} words total (~${estimatedTotalDuration}s duration).
- VIDEO ZONE covers the FIRST ~${videoWordBudget} words of the script (~${Math.round(videoDurationMin)} minutes).
- STATIC ZONE covers EVERYTHING from word ~${videoWordBudget + 1} onward (~${staticWordBudget} words, ~${Math.round(staticWordBudget / wpm)} minutes).
- Find the nearest sentence boundary around word ${videoWordBudget} to make a clean split.

The output must be divided into TWO zones:

🎥 VIDEO ZONE (first ${videoScenes} scenes):
- Covers the FIRST ~${videoWordBudget} words of the script.
- Each scene = ~8-second AI video clip → voiceOverText ~${wordsPerVideoScene} words per scene.
- Prefer SHORT, punchy sentences. Break compound sentences into simple ones.
- The visual prompt should describe MOTION and CAMERA MOVEMENT.

🖼️ STATIC ZONE (remaining ${staticScenes} scenes):
- Covers ALL remaining text from word ~${videoWordBudget + 1} to end (~${staticWordBudget} words).
- voiceOverText ~${wordsPerStaticScene} words per scene.
- Visual prompts describe a single still frame.

⚠️ RULES:
- Total scenes = ${videoScenes} (video) + ${staticScenes} (static) = ${totalScenes}
...
- CRITICAL: All ${totalScenes} scenes together must cover the ENTIRE script (~${wordCount} words). No gaps.
`;
            }
...
...
${videoZoneConfig?.enabled ? (() => {
                    const vDur = videoZoneConfig.videoScenes * 8;
                    const vWords = Math.round((vDur / 60) * wpm);
                    return `⚠️ VIDEO/STATIC SPLIT:
- The FIRST ~${vWords} words of the script → ${videoZoneConfig.videoScenes} [VIDEO] shots (~${Math.ceil(vWords / videoZoneConfig.videoScenes)} words each).
- Everything after word ~${vWords} → ${videoZoneConfig.staticScenes} [STATIC] shots.
- ALL script text must be covered. No text omitted.`;
                })() : ''}
...
2. Map the original script text (Voice Over) to these visual scenes.
   ...
   - 🚨 COMPLETE COVERAGE: The concatenation of ALL scenes' voiceOverText MUST reproduce the ENTIRE original script. No sentences may be skipped.
   - If you are running out of scenes, put remaining script text into the last scene(s) — do NOT drop it.
3. Extract Characters, Locations, and Chapters as usual.
4. FINAL CHECK: 
   a. Count your scenes array. If length > ${Math.ceil(expectedSceneCount * 1.1)}, go back and merge scenes.
   b. Verify ALL original script text is present in voiceOverText fields. Missing text = FAIL.

...
const IMPERIAL_CONFIG = {
    baseUrl: '/api/proxy/imperial',  // Via Vercel serverless proxy
    timeout: 120000,
    models: {
        // Text models
...
export async function callImperialImage(
    ...
    if (useGenerateEndpoint) {
        ...
        response = await fetch(`${IMPERIAL_CONFIG.baseUrl}/v1/images/generations`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${apiKey}`,
        ...
        response = await fetch(`${IMPERIAL_CONFIG.baseUrl}/v1/chat/completions`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${apiKey}`,
